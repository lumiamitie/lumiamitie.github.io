$(function() {                                    // DOM이 로드되면 실행한다
  ...
  function loadTimetable() {                    // 함수를 선언한다
    $.getJSON('example.json')              // JSON 데이터를 수집한다
    .done( function(data){                      // 데이터를 가져왔으면
      chapters = data;                             // 변수에 저장한다
  ...
  $('#content').on('click', '#event a', function(e) {  // 행사를 클릭했으면
    ...
    for (var i = 0; i < chapters[loc].length; i++) {      // 루프를 실행한다
      newContent += '<li><span class="time">' + chapters[loc][i].time + '</span>';
      newContent += '<a href="descriptions.html#';
      newContent += chapters[loc][i].title.replace(/ /g, '-') + '">';
      newContent += chapters[loc][i].title + '</a></li>';
    }
