...
d3.json("data/votes.json", function(data){
	...
	var nested_st = nested.map(function(d){
		return d.values.map(function(e){
			var total = d3.sum( // %를 계산하기 위해 지역별 투표수의 합을 구한다
							totalVotes.filter(function(d){return d.region === e.region})
							.map(function(d){return d.value})
						)
			return {x: e.region, y: e.value, t: e.variable, total: total}
			})
		})
	//testnest = nested_st
	...
	var rect = svg.selectAll("rect")
				...
				.attr("y", function(d, i){return y((d.y+d.y0)/d.total);})
				.attr("width",function(d){return x.rangeBand();})
				.attr("height", function(d, i){
					return y(d.y0/d.total) - y((d.y+d.y0)/d.total);
					})
	...
	var yAxis = d3.svg.axis()
			...
			.outerTickSize(0)
			.tickPadding(-5)
			.tickFormat(d3.format("%")); //눈금의 표시형식을 퍼센트로 지정
