...
d3.json("data/sentiment_new.json", function(senti) {

  	var keywords = d3.nest()
				.key(function(d){return d.sentiment;})
				.sortKeys(d3.ascending) //negative가 positive보다 앞에 오도록
				.entries(senti)
				
	var sentiments = d3.layout.stack()(keywords.map(function(d){
  ...
  y_max = d3.max([d3.max(keywords.map(function(d){return d.values.map(function(e){return e.count})})[0])
				,d3.max(keywords.map(function(d){return d.values.map(function(e){return e.count})})[1])])
